...
csv_file = '..\\Parsed Data\\product_groups.csv'  # Replace with the actual path to your product_groups.csv file

# Define batch size for APOC processing
BATCH_SIZE = 1000

def create_product_group_relationships(csv_file, uri, user, password):
    # Connect to Neo4j
    ...
    try:
        ...
        print(f"Creating PART_OF relationships in Neo4j using APOC batch processing...")
        ...
        for i in tqdm(range(0, len(rows), BATCH_SIZE), desc="Processing batches"):
            ...
            CALL apoc.periodic.iterate(
                ...
                'MATCH (p:Product {Id: row.Id}), (g:Group {name: row.group})
        ...
        print("PART_OF relationships created successfully in Neo4j.")
...
# Run the function
create_product_group_relationships(csv_file, NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD)