...
csv_file = '..\\Parsed Data\\product_categories.csv'  # Replace with the actual path to your product_categories.csv file

# Define batch size for APOC processing
BATCH_SIZE = 1000

def create_product_category_relationships(csv_file, uri, user, password):
    # Connect to Neo4j
    ...
    try:
        ...
        print(f"Creating BELONGS_TO relationships in Neo4j using APOC batch processing...")
        ...
        for i in tqdm(range(0, len(rows), BATCH_SIZE), desc="Processing batches"):
            ...
            CALL apoc.periodic.iterate(
                ...
                'MATCH (p:Product {Id: row.Id}), (c:Category {name: row.Category})
                 USING INDEX p:Product(Id)
                 USING INDEX c:Category(name)
        ...
        print("BELONGS_TO relationships created successfully in Neo4j.")
...
# Run the function
create_product_category_relationships(csv_file, NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD)